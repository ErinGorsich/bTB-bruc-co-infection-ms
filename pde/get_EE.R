...
get_EE_lhs = function(params, x0, method){
    ###################################
	# Input: x = c(S = final, 1*20 S vector at params)
	# method = "logistic", "ricker", "beverton-holt"
	# Output: Ro
	...
	Tot <- sum(sol[length(sol[,1]), 2:length(colnames(sol))])	
	EE_bTB_alone <- (It + Ic + Rc) / Tot
	xT <- unname(unlist(sol[length(sol[,1]), c(2:length(colnames(sol))) ] ))
	rm(sol)
	...
	EE_bruc_co <- (Ib + Ic + R + Rc) / Tot
	rm(sol)
	out <- list(EE = c(EE_bTB_alone, EE_bTB_co, EE_bruc_alone, EE_bruc_co), xB = xB, xT = xT)
	return(out)
}
