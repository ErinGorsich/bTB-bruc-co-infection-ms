...
#############################################################
#############################################################
rhs_optim = function(times, x, params){
	##########################
	# AS RHS BUT NOW DO NOT READ betapB or beatapT with input.
	# LOWER SABIE TRANSMISSION INCREASE !!!!!!!!!!!!!!!  
	# Inputs: t = time sequence; 
	# x = initial conditions
	# params= list(
	# b1, b2, b3, b4, b5-prop reduction in fecundity with TB, bruc, chronic bruc, co, coinfect/chronic 
	# K 
	# betaT, betaB- transmission rates, p fo co-inf first
	# gamma, epsilon	 
	...
	with(as.list(c(x, params)), {
		...
		lambdaB <- betaB * (Ib + Ic) 
		lambdapT <- betaT * (It + Ic + Rc)
		lambdapB <- 3.92 * betaB * (Ib + Ic) 
