...
############################################
############################################
# New data, no bolus- age category (Repeat above with age category)
############################################
############################################
# 1.4-1.9, 2-3.9, 4-8, 8+
...
test.mod<-coxph(Surv(start, stop, death.time)~brucella*age2.2+TB_3+herd2+ cluster(animal), data=data3); summary(test.mod) # AIC = 313.4
#data3$age2.2<- relevel(data3$age2.2, "subadult")
test.mod<-coxph(Surv(start, stop, death.time)~age2.2 + cluster(animal), data=data3); summary(test.mod)


data3$age2.3 <- data3$age2.2
data3$age2.3[data3$age_yr2 == 3] <- "juvenile"
test.mod<-coxph(Surv(start, stop, death.time)~brucella*age2.3+TB_3+herd2+ cluster(animal), data=data3); summary(test.mod) # AIC = 323.4489
...
test.mod<-coxph(Surv(start2, stop2, death.time)~ age6 + cluster(animal), data=data3); summary(final.mod)
